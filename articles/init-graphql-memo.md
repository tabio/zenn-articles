...
|ツール名|memo|
|---|---|
|[Insomnia](https://github.com/Kong/insomnia)|star多い|
...
- RESTのデメリット
  ...
  - GraphQL API は、クライアントがサーバに対して返却してほしいデータの構造を指定することができる。
    - RESTはクライアントが欲しい情報を、少ないリクエスト数で過不足なく取得するのが難しい。
  - APIのドキュメントは実装と密結合した形で管理される。そのため、実装と仕様が乖離しにくい。
